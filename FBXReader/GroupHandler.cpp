	...
	if (lAttributeType == FbxNodeAttribute::EType::eNull)
	{
		...
		while (prop != NULL)
		{
			...
			for (int i = 0; i < properties.size(); i++)
			{
				...
				//tmpGroup->groupInfo.dataType = properties[i].GetPropertyDataType().GetName();

				memcpy(tmpGroup->groupInfo.groupAttrName,attrname, sizeof(char)*256);

				memcpy(tmpGroup->groupInfo.dataType, datatype, sizeof(char)*256);
